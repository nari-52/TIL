...
public class jdbcTest01_DDL1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// >>> 5. Statement stmt 객체 (우편배달부)가 작성된 SQL문(편지)을 오라클서버에 보내어 실행되어지도록 한다.
			boolean isSQL1 = stmt.execute(sql1);
			boolean isSQL2 = stmt.execute(sql2);
			boolean isSQL3 = stmt.execute(sql3);
			/*
				stmt.excute(sql1); 을 실행하면 true 또는 flase가 나온다.
				파라미터로 들어오는 sql1문이 select 문 일때만 정상적으로 성공시 true를 리턴시켜주고,
...
//			확인용 isSQL1 => false // DDL 문이기 떄문에 성공해도 false가 리턴되어진다.
